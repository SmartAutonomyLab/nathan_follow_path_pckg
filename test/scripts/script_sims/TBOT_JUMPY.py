...
class plotter:
    ...
    def plot(self, arrival_times):
        ...
        plt.plot(self.x[0,:], self.x[1,:], color='b',linestyle = ':', label='estimate')
...
if __name__=="__main__":
    ...
    if JUMPY:
        ...
        R = (1e-1)*np.eye(num_state) # measurement noise
        dn = 5.     # measurement period
    else:
        x_cp   = 0. # x process covariance
        y_cp   = 0. # y process covariance
        yaw_cp = 0. # yaw process covariance
        Q       = dt * np.diag([x_cp, y_cp, yaw_cp])
        R = (1e-5)*np.eye(num_state) # measurement noise
        dn = 2.     # measurement period
    # CONTROL
    v = 0.20    # desired tbot radial speed (held fixed)
    kP = 5e-1  # P controller gain
    kI = 1e-2  # I controller gain
    kD = 2e-3  # D controller gain
